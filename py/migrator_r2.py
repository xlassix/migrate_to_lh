...
class Migrator:
    ...
    _bucket = boto3.resource('s3').Bucket("poda")
    ...
    @classmethod
    def connect_db(self,  bucket_name: str, key_id: str, secret: str, accountid: str):
        # Initialize S3 with provided credentials and bucket
        if bucket_name == '' or key_id == '' or secret == '' or accountid == "":
            raise Exception(
                "No token provided: Please provide a token or set the LIGHTHOUSE_TOKEN environment variable"
            )
        if bucket_name != '' and key_id != '' and secret != '' and accountid != "":
    ...
    def copy_all_files_from_s3_to_lighthouse(self, startAt: str = ""):
        ...
        for _object in self._bucket.objects.all():  # .filter(Marker=startAt):
...
if __name__ == "__main__":
    ...
    migrator = Migrator()
    migrator.connect_db(
        bucket_name, os.environ['AWS_ACCESS_KEY_ID'], os.environ['AWS_SECRET_ACCESS_KEY'], os.environ['ACCOUNT_ID'])
